...
import PrivacyOperationsPanel from './PrivacyOperationsPanel.vue'
...
<template v-if="runtimeSettingsReady">
    ...
    <section v-else-if="activeSettingsSection === 'diary'" class="settings-content-section">
      <template v-if="runtimeSettingsReady">
        <div class="settings-section-block"><h2>自动整理</h2><div class="settings-list">
          <label class="settings-item"><span><strong>自动生成每日成长日记</strong><small>逻辑日结束后生成；启动后逐天补写有记录的漏写日期，保留已有日记</small></span><span class="switch-control"><input v-model="runtimeSettingsDraft.daily_diary_auto_enabled" type="checkbox" /><i /></span></label>
          <label class="settings-item"><span><strong>日记专用模型</strong><small>日记整理使用此模型；不会更改日常聊天模型</small></span><select v-model="runtimeSettingsDraft.daily_diary_model_id" aria-label="日记专用模型"><option value="">沿用默认模型</option><option v-for="model in modelOptions" :key="model.id" :value="model.id">{{ model.provider_name || model.provider }} · {{ model.display_name || model.model }}</option></select></label>
          <label class="settings-item"><span><strong>检查间隔</strong><small>每次最多补写一天；生成失败时逐步延长重试间隔</small></span><span class="settings-number"><input v-model.number="runtimeSettingsDraft.daily_diary_check_seconds" type="number" min="30" max="3600" /><b>秒</b></span></label>
        ...
        <div class="settings-section-block"><h2>备份与夜间收尾</h2><div class="settings-list">
          <label class="settings-item"><span><strong>自动备份日记数据</strong><small>备份用于故障恢复，不在日记页面显示版本历史</small></span><span class="switch-control"><input v-model="runtimeSettingsDraft.backup_enabled" type="checkbox" /><i /></span></label>
           ...
           <label class="settings-item"><span><strong>自动备份总容量</strong><small>超过容量时优先删除最旧的自动备份，至少保留最新一份</small></span><span class="settings-number"><input v-model.number="runtimeSettingsDraft.backup_max_total_mb" type="number" min="64" max="102400" step="64" /><b>MB</b></span></label>
    ...
    <section v-else-if="activeSettingsSection === 'models'" class="settings-content-section">
      ...
      <div v-else class="settings-section-block"><h2>自动选择策略</h2><ModelStrategyPanel /><div class="settings-list"><div class="settings-item"><span><strong>简单日常对话</strong><small>优先选择低延迟模型，并保留模型默认思考</small></span><b class="settings-readonly">自动</b></div><div class="settings-item"><span><strong>复杂分析与任务</strong><small>按模型能力和原生思考档位提高推理强度</small></span><b class="settings-readonly">自动</b></div><div class="settings-item"><span><strong>降级顺序</strong><small>首选模型不可用时，依次尝试其他已连接模型</small></span><b class="settings-readonly">按优先级</b></div><div class="settings-item"><span><strong>费用</strong><small>优先显示供应商实扣；无法立即取得时显示“实扣待确认”并后台补账</small></span><b class="settings-readonly">后台补账</b></div></div></div>
    </section>
    ...
    <section v-else-if="activeSettingsSection === 'data'" class="settings-content-section">
      ...
      <div class="settings-section-block"><div class="settings-section-heading"><div><h2>完整备份与恢复</h2><p>包含数据库、日记、记忆、角色、模型配置、头像、背景和桌宠设置；自动与普通手动备份不重复包含可重新安装的大模型权重</p></div><span class="backup-heading-actions"><button type="button" :disabled="Boolean(dataPrivacyBusy)" @click="cleanupDataBackups"><Trash2 :size="15" />{{ dataPrivacyBusy === 'cleanup' ? '正在清理' : '清理旧备份' }}</button><label><Download :size="15" />{{ dataPrivacyBusy === 'import' ? '正在导入' : '导入备份' }}<input type="file" accept=".zip,application/zip" :disabled="Boolean(dataPrivacyBusy)" @change="importDataBackup" /></label><button type="button" :disabled="Boolean(dataPrivacyBusy)" @click="createDataBackup"><Archive :size="15" />{{ dataPrivacyBusy === 'create' ? '正在备份' : '创建完整备份' }}</button></span></div>
        <p v-if="dataPrivacyState.backupStorage" class="settings-note">当前共 {{ dataPrivacyState.backupStorage.count || 0 }} 份、{{ (Number(dataPrivacyState.backupStorage.total_bytes || 0) / 1073741824).toFixed(2) }} GB；自动备份上限 {{ dataPrivacyState.backupStorage.keep_count || 14 }} 份 / {{ Math.round(Number(dataPrivacyState.backupStorage.max_total_bytes || 0) / 1048576) || 1024 }} MB。</p>
        ...
        <div v-else class="backup-list"><article v-for="item in dataPrivacyState.backups" :key="item.name" :class="{ invalid: !item.valid }"><div><strong>{{ item.name }}</strong><small>{{ item.created_at || '未知时间' }} · {{ (Number(item.size || 0) / 1048576).toFixed(2) }} MB · {{ item.valid ? (item.includes_model_weights ? '校验通过 · 含模型权重' : '校验通过 · 轻量数据备份') : (item.error || '旧格式或损坏') }}</small></div><span><a v-if="item.valid" :href="`/api/backups/${encodeURIComponent(item.name)}/download`" download title="下载备份"><Download :size="15" /></a><button v-if="item.valid" type="button" title="恢复这个备份" :disabled="Boolean(dataPrivacyBusy)" @click="restoreDataBackup(item.name)"><RotateCw :size="15" /></button><button type="button" title="删除这个备份" :disabled="Boolean(dataPrivacyBusy)" @click="deleteDataBackup(item.name)"><Trash2 :size="15" /></button></span></article></div>
      </div>
